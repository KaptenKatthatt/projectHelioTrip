...
import { fetchHorizonsVectors, HorizonsError, } from './horizons.js';
import { HORIZONS_COMMAND_IDS, isMoonId, isPlanetId, MOON_IDS, MOON_META, PLANET_IDS, } from './planets.js';
...
const hasValidAnalyticsToken = (provided) => {
    if (ANALYTICS_ADMIN_TOKEN.length === 0)
        return false;
...
const horizonsUpstreamResponse = (error) => {
...
const respondWithEphemeris = async (context, resolveBody) => {
    try {
...
export const buildApp = () => {
    ...
    app.use('*', cors());
    app.get('/health', (c) => c.json({ status: 'ok', planets: PLANET_IDS, moons: MOON_IDS }));
    app.post('/analytics/event', async (c) => {
        ...
        try {
            await analyticsStore.recordAnalyticsEvent(name, value);
        }
        catch (error) {
    ...
    app.get('/planets/:id', async (c) => {
        ...
        if (!isPlanetId(id)) {
            return c.json({ error: 'unknown_planet', validIds: PLANET_IDS }, 400);
        }
        const date = parseDate(c.req.query('date'));
        if (!date) {
            return c.json({ error: 'invalid_date', hint: 'Use YYYY-MM-DD' }, 400);
        }
    ...
    app.get('/moons/:id', async (c) => {
        const id = c.req.param('id').toLowerCase();
        if (!isMoonId(id)) {
            return c.json({ error: 'unknown_moon', validIds: MOON_IDS }, 400);
        }
        const date = parseDate(c.req.query('date'));
        if (!date) {
            return c.json({ error: 'invalid_date', hint: 'Use YYYY-MM-DD' }, 400);
