...
const SUPABASE_INCREMENT_RPC = process.env.ANALYTICS_SUPABASE_INCREMENT_RPC?.trim() ||
...
export const isAnalyticsEventName = (raw) => VALID_EVENT_NAMES.has(raw);
const isAnalyticsAggregate = (entry) => {
...
export const eventValueFromPayload = (payload) => {
    ...
    if (typeof achievementId === "string")
        return normalizeValue(achievementId);
    const mode = payload.mode;
    if (typeof mode === "string")
        return normalizeValue(mode);
    const contextType = payload.context_type;
    if (typeof contextType === "string")
        return normalizeValue(contextType);
    ...
    if (typeof locale === "string")
        return normalizeValue(locale);
    const value = payload.value;
    if (typeof value === "string")
        return normalizeValue(value);
    return EMPTY_VALUE_LABEL;
};
export const recordAnalyticsEvent = async (name, value) => {
    writeQueue = writeQueue.then(async () => {
        try {
            ...
            const day = utcDay();
            const existing = store.aggregates.find((entry) => entry.date === day && entry.name === name && entry.value === value);
            if (existing) {
                existing.count += 1;
            }
            else {
...
export const readAnalyticsSummary = async () => {
    ...
    if (supabase) {
        ...
        const rows = (data ?? []).filter((entry) => isAnalyticsAggregate(entry));
