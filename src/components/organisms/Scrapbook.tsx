...
export const Scrapbook = ({ open, onClose }: ScrapbookProps) => {
  ...
  const selectedPhoto =
    selectedIndex !== null ? photos.at(selectedIndex) : undefined;

  return (
    <div className="pointer-events-auto fixed inset-0 z-[100] flex flex-col bg-black/95 p-4 pt-10 text-white sm:p-8 backdrop-blur-xl">
      <header className="mb-6 flex items-center justify-between">
        <div className="flex flex-col">
          <h2 className="text-2xl font-bold tracking-tight">Mitt Fotoalbum</h2>
          <p className="text-xs text-white/40 uppercase tracking-widest">{photos.length} foton sparade</p>
        </div>
        <button
          type="button"
          onClick={onClose}
      ...
      <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
        {photos.length === 0 ? (
          <div className="flex h-full flex-col items-center justify-center text-white/50">
            ...
            <p className="mt-1 text-sm opacity-60">Fånga universums skönhet med kameran så dyker dina foton upp här.</p>
          </div>
        ) : (
            ...
            {photos.map((photo, index) => (
              ...
              >
                ...
                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/90 to-transparent p-2 pt-6">
                  <p className="truncate text-[10px] font-bold uppercase tracking-wider">{photo.locationLabel}</p>
                </div>
              </button>
            ))}
          </div>
      ...
      {selectedPhoto ? (
        <div className="fixed inset-0 z-[110] flex flex-col bg-black/98 backdrop-blur-2xl animate-in fade-in zoom-in duration-300">
          ...
          <div className="relative flex flex-1 items-center justify-center p-4">
            ...
            <div className="relative max-h-full max-w-5xl overflow-hidden rounded-2xl shadow-2xl shadow-black/50 ring-1 ring-white/10">
              <img
              ...
              <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent p-6 pt-20">
                <h3 className="text-2xl font-black uppercase tracking-tighter text-white">{selectedPhoto.locationLabel}</h3>
                <p className="text-sm font-medium text-white/50">
                  Taget den {new Date(selectedPhoto.timestampMs).toLocaleDateString()} kl {new Date(selectedPhoto.timestampMs).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                </p>
              </div>
            </div>
          ...
          <div className="p-6 text-center text-xs font-bold uppercase tracking-[0.3em] text-white/20">
