...
export const TimeScrubber = () => {
  ...
  return (
    <div className="pointer-events-auto mx-auto w-full max-w-3xl rounded-2xl border border-white/10 bg-black/40 px-3 py-3 backdrop-blur-md sm:px-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-1.5">
          ...
          <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-1 text-xs">
            <span className="font-mono text-white/80">{dateFmt.format(simTime)}</span>
        ...
        <div className="flex flex-wrap items-center gap-1 rounded-xl border border-white/10 bg-white/5 p-1 sm:shrink-0 sm:flex-nowrap">
          {SPEEDS.map((s) => (
            <button
              key={s}
              type="button"
