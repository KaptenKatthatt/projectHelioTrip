...
const extractFrontendEventNames = (): string[] => {
  const src = readFileSync(path.resolve(__dirname, '../../src/lib/analytics.ts'), 'utf8');
  const match = src.match(/type AnonymousEventName\s*=\s*([\s\S]+?);/);
  const matchGroup = match?.[1];
  if (!matchGroup)
    throw new Error('Could not find AnonymousEventName type in src/lib/analytics.ts');
  const names: string[] = [];
  const re = /"([^"]+)"/g;
  let m: RegExpExecArray | null;
  while ((m = re.exec(matchGroup)) !== null) {
    const eventName = m[1];
    if (eventName) {
